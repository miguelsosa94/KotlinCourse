...
/*
    ...
    try {
        ...
        val countries : Array<String> = arrayOf("India","Mexico","Colombia","Argentina","Chile","Perú")

        val days : Array<String> = arrayOf<String>("Lunes","Martes","Miercoles")
        var numbers = intArrayOf(5,6,7,8)

        days.forEach { println(it) }
        numbers = numbers.plus(9)
        println("Promedio: ${numbers.average()}")
        println("Suma: ${numbers.sum()}")
        numbers = numbers.reversedArray()
        numbers.forEach { println("Reversa: $it") }
